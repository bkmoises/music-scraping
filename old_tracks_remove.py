...
def main(user_id: str, refresh_token: str, client_id: str, client_secret: str):
    ...
    playlist_uri = spotify.get_playlist_uri(user_id, token, "New Rock Hits")
    tracks       = spotify.get_playlist_tracks_metadata(playlist_uri, token)
    ...
    for track in tracks:
        ...
        if added_at:
            ...
            if added_at_dt < limit:
                tracks_to_remove.append({"uri": track.get("uri")})
                logging.info(f"Removendo faixa: {track.get('name')} - {','.join(track.get('artists'))} adicionada em {added_at_dt.isoformat()}")
                
    if tracks_to_remove:
        spotify.remove_tracks(playlist_uri, tracks_to_remove, token)
        return 0
